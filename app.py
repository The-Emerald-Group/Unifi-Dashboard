...
def send_email(subject, html_body, log_identifier):
    ...
    for attempt in range(1, 4):
        ...
        except Exception as e: log(f"!! Email attempt {attempt} failed: {str(e)}"); time.sleep(5)
    return False
...
def fetch_modern_unifi(alert_state, pending_offline, pending_recovery):
    ...
    try:
        ...
        for host_group in dev_res:
            ...
            # THE HEARTBEAT SENSOR: deviceStateLastChanged from your raw JSON dump
            ...
            for d in devices_list:
                ...
                if dev_status != "online":
                    # Suppression logic for transitional states
                    ...
                    # If the app heartbeat is fresh, ignore gateway's "offline" report
            ...
            for iss in isp_periods:
                # SKIP high latency and ghost stutters
            ...
            if valid_buckets >= 2:
                if weight < 15: status, weight = "Yellow", 15; issues.append({"label": "📡 RECENT ISP ISSUE", "time": "< 2h ago", "severity": "warning"})

            if recent_devs > 0 and weight < 10:
                status, weight = "Yellow", 10; issues.append({"label": f"⚠️ {recent_devs} Device(s) Offline", "time": "Partial", "severity": "warning"})
    ...
    except Exception as e: log(f"!! API Error: {str(e)}")
    return cards
...
def fetch_classic_unifi(alert_state, pending_offline, pending_recovery):
    ...
    try:
        ...
        for site in sites_res:
            ...
            status, weight, issues, inventory, recent_devs, hist_devs = "Green", 0, [], [], 0, 0
            for dev in dev_res:
...
def harvest_data():
    ...
    while True:
        ...
        all_cards = fetch_modern_unifi(alert_state, pending_off, pending_rec) + fetch_classic_unifi(alert_state, pending_off, pending_rec)
